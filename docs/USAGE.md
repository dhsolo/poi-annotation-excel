...
- **多 Builder 多 Sheet**：`ExcelUtil.toBytes(builder1, builder2)` / `ExcelUtil.export(out, name, builder1, builder2)`，每个 Builder 一个 Sheet。
- **复杂/父子 Sheet**：模型实现 `ComplexExcelModel` 可递归生成子表；父表的图片处理器、行号、样式在子表间共享。
- 子表会复用父工作簿：并入时样式按父工作簿重建，子表**单独设置的自定义标题/表头/数据样式会被克隆保留**（不再被重置为默认）。
- **`@ExcelInfoChild`**：把嵌套子对象的列拍平进父行；**支持多层嵌套**（路径如 `addr.geo.zip`），导入按路径重建各级对象。
